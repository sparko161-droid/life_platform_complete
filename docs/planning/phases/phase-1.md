...
## UX contract
Core screens must be connected by explicit contracts: entry/exit, data, action, canonical application operation, state transition, next navigation, error/empty/offline handling and Russian-only visible text. Child path is `Мой день → Задание → Выполнение → Результат → Мой день`.

## Contract gate
Task DSL, verification result, reward event and core screen/API schemas freeze before parallel UX, evidence and mobile streams consume them.
